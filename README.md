...
- `F3`: 打开配置编辑窗口，可新增命令配置或修改当前命令的分类、标题、模板和参数列表
- `?`: 在普通模式打开或关闭快捷键提示窗口
...
按 `F3` 可以打开配置编辑窗口。默认会载入当前选中的命令；`Ctrl+n` 新建命令草稿，`Enter` 编辑当前字段，`Ctrl+s` 保存。保存目标是 `~/.config/cmdp/zz_cmdp_editor.toml`，文件名靠后排序，因此它可以覆盖普通全局配置里的同 ID 命令；项目本地 `.cmdp.toml` 仍然最后加载，优先级最高。

配置编辑器的 `参数` 字段使用逗号分隔，支持 `name` 或 `name:label` 两种写法，例如 `path:路径, pattern:匹配内容`。保存后会按普通配置规则校验命令 ID、分类 ID、参数名和模板语法。
