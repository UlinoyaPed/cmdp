...
配置编辑器会在命令 ID 旁显示命令标题，在分类 ID 旁显示分类别名。它还会从 `命令模板` 字段解析 `<<...>>` 和 `[[...]]` 片段，并在字段列表下方显示可点击的片段行；片段行会优先显示参数或选项的 `label` 别名，没有别名时才显示原始 ID。点击 `<<{{path}}>>` 这类必填片段，会弹出参数属性窗口，可以编辑对应参数的 `label`、`default`、`placeholder`、`help`、`secret` 和 `choices`；点击 `[[glob:--glob {{glob}}]]` 这类可选片段，可以编辑可选项的 `label`、`default_enabled`，以及片段内参数的同类属性。属性变更会自动写回配置草稿里的 `params` / `options` 字段，再按 `Ctrl+s` 保存。
