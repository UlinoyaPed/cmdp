...
1. 优先使用系统的 XDG 配置目录（通常是 `${XDG_CONFIG_HOME:-~/.config}/cmdp/`）。如果该目录不存在而旧的 `~/.config/cmdp/` 已存在，则继续使用旧目录；两者都存在时只使用 XDG 目录，不自动合并或覆盖。
...
保存目标会显示在配置编辑窗口标题中。编辑已有命令时会写回该命令实际来源的全局文件或项目 `.cmdp.toml`；新建全局命令写入 `zz_cmdp_editor.toml`。保存采用保留格式的 TOML 编辑和原子替换，未编辑的注释、空行、字段顺序及未知扩展字段会保留。修改命令 ID 是真正的重命名：旧键会被移除；如果新 ID 已被同一文件中的其他命令占用，保存会失败且原文件不变。
...
只有 `name` 必填。焦点在参数面板时，`help` 会在底部随当前选中参数显示，适合解释 choices、危险模式、路径格式或单位；`placeholder` 仍显示在输入框内或参数行尾部，适合放短示例。普通 `{{name}}` 参数在 Unix 上会自动转换为 POSIX Shell 单引号字面量，空格、引号、`$()`、反引号、分号和换行都会作为普通数据传入，不要再依赖模板中的双引号进行安全处理。旧的 `"{{name}}"` 写法会兼容识别。参数仍不做类型、路径或数字校验。
...
配置校验要求模板中使用的参数都有对应 `params` 定义、每个可选片段和 `options` 双向对应；保留但尚未在模板中使用的参数定义目前允许存在。重复使用同一个可选片段 ID 表示多个片段共享同一个开关，匿名片段 ID 按模板中的出现顺序稳定生成为 `option_1`、`option_2` 等。`choices` 不得为空，默认值必须属于 choices；秘密参数不能使用 choices。空模板和纯空白模板会被拒绝。

## 模板语法

- `{{name}}`: 默认安全转义的普通用户参数
- `{{{name}}}`: 显式原始 Shell 片段，仅用于确实需要操作符、重定向或表达式的高级参数
- `<<...>>`: 必填片段，片段内参数缺失时不能确认执行
...
配置文件中的 `template` 本身会作为可信 Shell 代码执行，可以包含管道、重定向和多行脚本；用户填写的普通参数则按不可信数据处理并自动转义。只有使用三花括号 `{{{raw_args}}}` 才会主动跨过这条边界，例如 `cargo build {{{extra_flags}}}`。原始参数能够执行任意 Shell 语法，配置作者必须自行保证其来源可信。

`secret = true` 会让参数在预览、执行前打印文本和错误上下文中脱敏，并阻止它写入状态文件。真实值仍会交给 Shell 执行，因此它可能通过目标程序参数、进程列表、Shell 行为或操作系统审计暴露；需要更强保护时应优先使用标准输入、环境变量或专用凭据机制。
...
按 `Ctrl+y` 后，`cmdp` 会先打印脱敏后的展示命令，然后用当前 Shell 执行真实命令，并把子进程的 `stdin`、`stdout`、`stderr` 直接继承到当前终端。Unix 使用 `$SHELL -c`（未设置时使用 `/bin/sh`），子进程被信号终止时按 `128 + signal` 返回。当前版本不声称能够为 `cmd.exe` 提供完整安全转义，因此 Windows 构建可以检查和运行非执行逻辑，但会明确拒绝启动模板命令。项目使用 stable Rust，CI 同时检查 Linux 与 Windows。
