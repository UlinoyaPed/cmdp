...
按 `F3` 可以打开配置编辑窗口。默认会载入当前选中的命令；`Ctrl+n` 新建命令草稿，`Enter` 编辑当前字段，`Ctrl+s` 保存。也可以直接用鼠标点击字段进入编辑。保存目标是 `~/.config/cmdp/zz_cmdp_editor.toml`，文件名靠后排序，因此它可以覆盖普通全局配置里的同 ID 命令；项目本地 `.cmdp.toml` 仍然最后加载，优先级最高。
